...
// index.ts
async function fetchHistoricalDaily(symbol, from, to2 = /* @__PURE__ */ new Date()) {
  const period1 = Math.floor(from.getTime() / 1e3);
  const period2 = Math.floor(to2.getTime() / 1e3);
...
__name(fetchHistoricalDaily, "fetchHistoricalDaily");
function calculateSMA(prices, period) {
  if (prices.length < period) return null;
  const slice = prices.slice(-period);
  return slice.reduce((sum, p2) => sum + p2, 0) / period;
}
...
function calculateEMA(prices, period) {
  ...
  const ema = [];
  const firstSMA = prices.slice(0, period).reduce((sum, p2) => sum + p2, 0) / period;
  ema.push(firstSMA);
...
function hasLowerHighs(prices, count = 3) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    recentHighs.push(Math.max(...chunk.map((p2) => p2.high)));
  }
...
__name(hasLowerHighs, "hasLowerHighs");
function hasHigherLows(prices, count = 3) {
  if (prices.length < count * 5) return false;
  const recentLows = [];
  const chunkSize = Math.floor(prices.length / count);
  for (let i = 0; i < count; i++) {
    const chunk = prices.slice(i * chunkSize, (i + 1) * chunkSize);
    recentLows.push(Math.min(...chunk.map((p2) => p2.low)));
  }
...
__name(hasHigherLows, "hasHigherLows");
function detectConsolidation(prices, lookback = 20) {
  if (prices.length < lookback) return false;
  const recent = prices.slice(-lookback);
  const rangeHigh = Math.max(...recent.map((p2) => p2.high));
  const rangeLow = Math.min(...recent.map((p2) => p2.low));
  const rangePercent = (rangeHigh - rangeLow) / rangeLow * 100;
  return rangePercent < 15;
}
__name(detectConsolidation, "detectConsolidation");
function detectWyckoffPhase(prices) {
  if (prices.length < 50) {
    const currentPrice2 = prices[prices.length - 1]?.close || 0;
    return {
      phase: "accumulation",
      subPhase: "A",
      strength: 0,
      support: currentPrice2 * 0.95,
      resistance: currentPrice2 * 1.05,
  ...
  const sortedPrices = [...prices].sort((a2, b) => a2.date.getTime() - b.date.getTime());
  const closes = sortedPrices.map((p2) => p2.close);
  ...
  const recentHigh = Math.max(...recent50.map((p2) => p2.high));
  const recentLow = Math.min(...recent50.map((p2) => p2.low));
  ...
  const isDowntrending = lowerHighs || ma20 && ma50 && currentPrice < ma20 && currentPrice < ma50 && ma20 < ma50;
  const isUptrending = higherLows || ma20 && ma50 && currentPrice > ma20 && currentPrice > ma50 && ma20 > ma50;
...
__name(detectWyckoffPhase, "detectWyckoffPhase");
function calculateTargets(currentPrice, phase, support, resistance, ma20, ma50) {
  switch (phase) {
    case "accumulation":
      return {
...
async function handleCron(env) {
  ...
  const sql2 = Ys(env.DATABASE_URL);
  const db = drizzle(sql2);
  try {
    const tickersResult = await sql2`
    ...
    for (const ticker of tickersResult) {
      try {
        ...
        const prices = await fetchHistoricalDaily(ticker.symbol, fromDate);
        if (prices.length === 0) {
